...
pub trait ChatCommand {
    ...
    fn handle(&self, message: &Message, discord: &Discord, config: &Config) -> Result<Message>;
}
...
impl Commands {
    ...
    pub fn handle(&self, message: &Message, discord: &Discord, config: &Config) {
        ...
        if is_command.is_match(&message.content) {
            ...
            for command in &self.commands {
                if command.matches(stripped.trim()) {
                    let result = command.handle(message, discord, config);
